// lib/embed/bridge.ts
// Purpose: this app's side of the embed on craudiovizai.com/apps/scrapbook.
// Date: 2026-09-10
//
// Javari Scrapbook keeps its own repo, Vercel project and tables. craudiovizai.com
// shows it inside a page on the site, so the site's header, footer and Javari
// widget surround it. This module is the contract between the two windows.
//
...
// Core preview deployments, so the embed can be verified before production.
// Only this team's deployment URLs match; the frame-ancestors CSP allows them on
// PREVIEW builds only (see next.config.js).
...
export function isTrustedParentOrigin(origin: string): boolean {
  return PARENT_ORIGINS.has(origin) || (process.env.NEXT_PUBLIC_VERCEL_ENV === 'preview' && PREVIEW_PARENT.test(origin));
}
